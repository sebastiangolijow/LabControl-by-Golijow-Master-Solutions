...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'📄 Archivo cargado: {len(practices_data)} prácticas encontradas')
        )
            ...
            for practice_data in practices_data:
                try:
                    ...
                    if created:
                        created_count += 1
                        self.stdout.write(
                            self.style.SUCCESS(f'  ✅ Creada: {practice.name} (${practice.price})')
                        )
                    else:
                        updated_count += 1
                        self.stdout.write(
                            self.style.WARNING(f'  🔄 Actualizada: {practice.name} (${practice.price})')
                        )

                except Exception as e:
                    error_count += 1
                    self.stdout.write(
                        self.style.ERROR(f'  ❌ Error con {practice_data.get("name", "Unknown")}: {str(e)}')
                    )
