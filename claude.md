...
### Done recently
- ✅ **2026-04-28 (later same day)** — Patient notification dispatch from `sync_labwin_results`. New `Study.notification_sent_at` field + new `apps.notifications.tasks.send_studies_available_email` (batched). Sync now ends with one email per patient: existing users → "your N new studies are available" (reuses `result_ready.html`), users newly created from PACIENTES with email → "set up your password" (reuses existing `send_password_setup_email`). Patients without email skipped silently — their studies stay `notification_sent_at=NULL` so a later sync can retry once the email is added. The DB flag prevents re-notification under the rolling 2-day window. 469 tests passing (5 new in `SyncNotificationTests`).
- ✅ **2026-04-28** — Rolling-window sync (initial 90 days + rolling 2 days). New settings `LABWIN_SYNC_INITIAL_DAYS=90` / `LABWIN_SYNC_ROLLING_DAYS=2`. First sync backfills the last 90 days; every subsequent sync re-scans the last 2 days so late-validated rows get picked up. Re-syncs are idempotent — `_get_or_create_study_with_practices` already updates `is_paid`, `is_validated`, and `RESULT_FLD` when source state changes (verified). Skips the lab's 14-year history per business decision.
